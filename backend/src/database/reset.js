...
async function resetDatabase() {
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.error('❌ ERRO: Reset de banco não permitido em ambiente de PRODUÇÃO!');
      console.error('🔒 Por segurança, este comando só pode ser executado em desenvolvimento.');
      process.exit(1);
    }
...
async function resetComplete() {
  try {
    // Verificar ambiente
    if (process.env.NODE_ENV === 'production') {
      console.error('❌ ERRO: Reset completo não permitido em ambiente de PRODUÇÃO!');
      console.error('🔒 Por segurança, este comando só pode ser executado em desenvolvimento.');
      process.exit(1);
    }
